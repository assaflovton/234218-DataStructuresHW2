...
template <class T>
void RankTree<T>::printInOrder(AVLNode<T>* root) const
{
    if (root)
    {
...
template <class T>
void RankTree<T>::fixNodeParent(AVLNode<T>* root, const T& value) const
{
    if (root != NULL)
    {
...
template <class T>
int RankTree<T>::getHeight(AVLNode<T>* root) const
{
    int height = 0;
    if (root != NULL)
    {
...
template <class T>
void RankTree<T>::fixParent(AVLNode<T>* root)
{
    if (root != NULL)
    {
        if (root->getParent())
...
template <class T>
AVLNode<T>* RankTree<T>::minValueNode(AVLNode<T>* node)
{
